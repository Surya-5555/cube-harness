...
from cube_harness.llm import LLMConfig, Prompt, RolloutLLMConfig
...
class TirAgentConfig(AgentConfig):
    ...
    def agent_name(self) -> str:
        return f"TIRAgent-{self.llm_config.model_name}".replace("/", "_")

    def make(self, action_set: list[ActionSchema] | None = None, **kwargs) -> "TirAgent":
        return TirAgent(config=self, tools=action_set or [])


class TirAgent(Agent):
    name: str = "tir_agent"
    description: str = "A minimal Tool-Integrated Reasoning agent that iterates over tool calls."
    ...
    def step(self, obs: Observation) -> AgentOutput:
        ...
        remaining = self.max_tokens - prompt_tokens
        if remaining < self.config.min_generation_tokens:
            logger.warning(
                "Prompt length %d leaves only %d tokens for generation (max_tokens=%d), stopping loop",
                prompt_tokens,
                remaining,
                self.max_tokens,
            )
        ...
        if max_tokens_this_turn < self.max_completion_tokens:
            logger.warning(
                "capping max_tokens from %d to %d (prompt_len=%d, max_tokens=%d)",
